...
function show_help() {
    ...
    echo "  logs-sql            - Tail SQL Server logs"
    echo "  logs-kafka          - Tail Kafka logs"
    echo "  port-forward-sql    - Port forward SQL Server (1433:1433)"
...
case "${1:-help}" in
    ...
    logs-consumer)
        ...
        ;;
    logs-sql)
        kubectl logs -f statefulset/mssql-server -n $NAMESPACE
        ;;
    logs-kafka)
    ...
    port-forward-sql)
        ...
        kubectl port-forward -n $NAMESPACE svc/mssql-server 1433:1433
        ;;
    port-forward-debez)
    ...
    check-connector)
        ...
        curl -s http://localhost:8083/connectors/mssql-cdc-connector/status | jq
        ;;
    test-data)
        echo "Inserting test data..."
        sqlcmd -S localhost,1433 -U sa -P 'YourSecureP@ssw0rd!' << EOF
USE testdb;
...
EOF
    ...
    describe-topic)
        if [ -z "$TOPIC" ]; then
            echo "Please set TOPIC environment variable: export TOPIC=sqlserver.dbo.Customers"
            exit 1
        fi
        POD=$(kubectl get pods -n $NAMESPACE -l app=kafka-broker -o jsonpath='{.items[0].metadata.name}')
        kubectl exec -it $POD -n $NAMESPACE -- /usr/local/bin/kafka-topics \
